...
.data
	n: .word 0 # four byte space registered for N
	n_square: .word 0 # another 4 byte space registered for N^2
...
main:
	...
	syscall
	# store first input (N) in $t2 for the time being
	move $t2, $v0
	...
	# Display the success message and then rpint the result values
	li $v0, 4 # 4 is to display a string
	la $a0, output
	syscall 	
	
	# store first input (N) in $t2 for the time being
	move $a0, $t2 # we can use $a0 again after storing it so that we could print the output message
